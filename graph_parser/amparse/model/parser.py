...
class AMParser(nn.Module):
    ...
    def __init__(self, config: AMParserConfig, numericalizer: AMNumericalizer):
        ...
        # for name, module in self.rel_biaff.items():
        #     print(f"Module: {name}")
    ...
    def span_predict(self, exs: torch.Tensor, span_out: torch.Tensor, mask: torch.Tensor,
        ...
        logits = [torch.amax(F.softmax(o[m], dim=-1), dim=-1) for o, m in zip(span_out, mask)]

        span_exs, span_mask, top_mask, spans = [], [], [], []
        spans_score = []
        for ex, outs, m, logit in zip(exs, arg_outs, mask, logits):
            assert len(outs) != 0
            ...
            if bio_spans:
                ...
                span_score = torch.stack([torch.mean(logit[s], dim=0) for s in bio_spans]).tolist()  # (n_span)
                # span_score = torch.cat((span_score, torch.tensor([0]).to(self.device)), dim=0) # (n_span+1)
        ...
        return {
            ...
            'span_score': spans_score # batch x n_span
        }
    ...
    def proposition_predict(self, prop_out: torch.Tensor, span_mask: torch.Tensor, fw: str): # -> List(str):
        ...
        pred_score = [torch.amax(F.softmax(o[m], dim=-1), dim=-1).tolist() for o, m in zip(prop_out, span_mask)]
    ...
    def load_model(cls, path,
        ...
        # print("1. numericalizer in load model: ", numericalizer)
        ...
        # print("2. numericalizer in load model: ", numericalizer)
