...
def ensemble(models: List[AMParser], batch, oracle_span: bool = False):
    ...
    for fw in repr_model.fw_vocab.tokens:
        ...
        prop_pred, prop_score_pred = repr_model.proposition_predict(prop_out=prop_out, span_mask=span_pred['span_mask'], fw=fw)
        ...
        fw_outs[fw] = {
            ...
            'span_score': span_pred['span_score'],  # batch x seq
            'prop': prop_pred,  # batch x n_span
            'prop_score': prop_score_pred,  # batch x n_span
    ...
    for i, _ in enumerate(input_target['tokens']):
        ...
        for fw in repr_model.fw_vocab.tokens:
            ...
            out[fw] = {
                ...
                'span': spans,
                'span_score': fw_outs[fw]['span_score'][i],
                'proposition': fw_outs[fw]['prop'][i],
                'proposition_score': fw_outs[fw]['prop_score'][i],
...
@torch.no_grad()
def ensemble_span_only(models: List[AMParser], batch, oracle_span: bool = False):
    assert len(models) > 0
