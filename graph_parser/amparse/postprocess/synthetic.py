...
@BasePostProcess.register("synthetic")
class SyntheticPostProcess(BasePostProcess):
    def reconstruct(self, sentence: Sentence, pred_data: Dict, edge_vocab: AMVocab):
        ...
        nodes_score = []
        for span, span_score, prop, prop_score in zip(pred_data['span'], pred_data['span_score'], pred_data['proposition'], pred_data['proposition_score']):
            nodes_score.append({
                'id': len(nodes),
                'anchors': [{"from": span.start, "to": span.stop}],
                'anchors_score': [span_score],
                'label': prop,
                'label_score': prop_score,
            })
            ...
            for frm, to in links:
                ...
                if frm == head:
                    ...
                    edges_score.append({
                        'source': frm,
                        'target': to,
                        'link_score': (float(arc[frm, to]), float(arc[to, frm])),
                        'label': rel_label,
                        'label_score': float(rel[frm, to, rel_id]),
                    })
