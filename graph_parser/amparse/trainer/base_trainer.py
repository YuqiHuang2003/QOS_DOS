...
class BaseTrainer:
    ...
    def run_train(self, train_loader: AMDataLoader, valid_loader: AMDataLoader, test_loader: AMDataLoader):
        ...
        for epoch in range(1, self.config.epochs + 1):
            ...
            print(f'Epoch finished: time={int(time.time()-start_time)}')
            ...
            if len(valid_loader) > 0:
                ...
                # val_f = val_scores['mix']['anchors']['f'] # note !!!!!!!!!!!!!!!
            ...
            if val_f >= best_f:
                ...
                if len(test_loader) > 0:
                    ...
                    if best_test_f < test_scores["all"]:
                        best_test_f = test_scores["all"]
                        best_test_e = epoch

                    logging.info(f'Test finished: '
                                    f'time={int(time.time() - start_time)}')
                    print(f'Test finished: time={int(time.time() - start_time)}')
                    ...
                    logging.info(f'[Test] Scores: {test_scores} at epoch {epoch}')
        ...
        logging.info(f'[Test] Best Scores: {best_test_f} at epoch {best_test_e}')
