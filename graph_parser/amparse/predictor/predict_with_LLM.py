...
from amparse.loader.data_loader import construct_loader, AMDataset
from amparse.model import parser_generator_with_LLM as generator_with_LLM
...
class AMPredictor_with_LLM:
    ...
    def predict_and_build_metric(self, models, numericalizer, config: AMParserConfig, loader):
        ...
        for i_batch, batch in enumerate(loader):
            pred_data = generator_with_LLM.ensemble_span_only(models=models, batch=batch['batch'], oracle_span=self.args.oracle_span)
            batch_origin = copy.deepcopy([sentence.data for sentence in batch['origin']])
            new_batch=[]
            for data, d in zip(batch_origin, pred_data):
                data["nodes"] =[{"id":i,"label":"None","anchors":[{'from': slice_obj.start, 'to': slice_obj.stop}]} for i, slice_obj in enumerate(d['synthetic']['span'])]
                data["edges"] = []
                data["tops"] = []
                new_batch.append(json.dumps(data))
            
            new_batch=AMDataset(corpus=Corpus.load_from_dump(dumps=new_batch), numericalizer=self.repr_numericalizer)
    ...
    def predict(self):
        ...
        # 保存为 JSON 文件
        with open(os.path.join(self.args.log, "args.json"), "w", encoding="utf-8") as json_file:
            json.dump(args_dict, json_file, indent=4, ensure_ascii=False)
...
def main():
    ...
    # config.log = "/".join(config.model.split("/")[:-1]) + "/prediction/"
    predictor = AMPredictor_with_LLM(args=config)
    predictor.predict()
    return
