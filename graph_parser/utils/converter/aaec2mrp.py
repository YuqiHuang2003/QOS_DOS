...
def main(conf: Arguments):
    ...
    train_mrps, test_mrps = split_dataset(conf=conf, mrps=mrps)
    
    random.seed(conf.seed)
    print(f'original train_mrps length = {len(train_mrps)}')
    
    if conf.dev_rate > 0:
        train_idx = list(range(len(train_mrps)))   
        random.shuffle(train_idx)
        vali_idx = train_idx[:int(len(train_mrps) * conf.dev_rate)]
        vali_mrps = [train_mrps[i] for i in vali_idx]
        train_mrps = [train_mrps[i] for i in train_idx[len(vali_idx):]] 
        print(f'train_mrps length = {len(train_mrps)}, vali_mrps length = {len(vali_mrps)}')
#         vali_path = f'./data_mrp/aaec/aaec_{conf.level}_vali.mrp'
        vali_path = os.path.join(conf.dir_output,f'aaec_{conf.level}_vali.mrp')
    ...
    if conf.low_res_rate > 0:   
        train_idx = list(range(len(train_mrps)))   
        random.shuffle(train_idx)
        low_res_idx = train_idx[:int(len(train_mrps) * conf.low_res_rate)]
        train_mrps = [train_mrps[i] for i in low_res_idx]
        print(f'after low resource train_mrps length = {len(train_mrps)}')

    print(f'test_mrps length = {len(test_mrps)}')

#     for path, mrps in zip([f'./data_mrp/aaec/aaec_{conf.level}_train.mrp', f'./data_mrp/aaec/aaec_{conf.level}_test.mrp'], [train_mrps, test_mrps]):
    for path, mrps in zip([os.path.join(conf.dir_output,f'aaec_{conf.level}_train.mrp'), os.path.join(conf.dir_output,f'aaec_{conf.level}_test.mrp')], [train_mrps, test_mrps]):
        os.makedirs(os.path.dirname(path), exist_ok=True)
        util.dump_jsonl(fpath=path, jsonl=mrps)
